...
# Собираем метрики
def collect_metrics():
    def iter_pages(initial_url):
        """Итерирует страницы ответа NetBackup, следуя ссылкам next без накопления в памяти."""
        url = initial_url
        visited = set()
    ...
    def job_collect():
        ...
        for page_jobs in iter_pages(jobs_url):
    ...
    def storage_collect():
        ...
        for page_storage in iter_pages(storage_url):
            for storage in page_storage:
